...
def test_init():
    ...
    with pytest.raises(ValueError):
        cosmo = core.FlatLambdaCDM(H0=70, Om0=0.27, Tcmb0=[0.0, 2])
    with pytest.raises(ValueError):
        h0bad = [70, 100]
...
def test_basic():
    ...
    assert np.allclose(cosmo.Tcmb0, 2.0)
    assert np.allclose(cosmo.Tnu0, 1.4275317, rtol=1e-5)
    assert np.allclose(cosmo.Neff, 3.04)
    assert np.allclose(cosmo.h, 0.7)
    assert np.allclose(cosmo.H0, 70.0)
...
def test_distance_broadcast():
    ...
    cosmo = core.FlatLambdaCDM(H0=70, Om0=0.27, m_nu=[0.0, 0.1, 0.011])
    ...
    methods = ['comoving_distance', 'luminosity_distance',
               ...
               'absorption_distance', 'Om', 'Ode', 'Ok', 'H',
...
def test_clone():
    ...
    cosmo = core.FlatLambdaCDM(H0=70, Om0=0.27, Tcmb0=3.0)
    ...
    newclone = cosmo.clone(H0=60)
    assert newclone is not cosmo
    assert newclone.__class__ == cosmo.__class__
    assert newclone.name == cosmo.name
    assert not np.allclose(newclone.H0, cosmo.H0)
    assert np.allclose(newclone.H0, 60.0)
    ...
    assert np.allclose(newclone.Tcmb0, cosmo.Tcmb0)
    assert np.allclose(newclone.m_nu, cosmo.m_nu)
    assert np.allclose(newclone.Neff, cosmo.Neff)

    # Compare modified version with directly instantiated one
    cmp = core.FlatLambdaCDM(H0=60, Om0=0.27, Tcmb0=3.0)
    assert newclone.__class__ == cmp.__class__
    assert newclone.name == cmp.name
    assert np.allclose(newclone.H0, cmp.H0)
    ...
    assert np.allclose(newclone.m_nu, cmp.m_nu)
    assert np.allclose(newclone.Neff, cmp.Neff)
    assert np.allclose(newclone.Om(z), cmp.Om(z))
    assert np.allclose(newclone.H(z), cmp.H(z))
    assert np.allclose(newclone.luminosity_distance(z),
                       cmp.luminosity_distance(z))

    # Now try changing multiple things
    newclone = cosmo.clone(name="New name", H0=65, Tcmb0=2.8)
    assert newclone.__class__ == cosmo.__class__
    assert not newclone.name == cosmo.name
    assert not np.allclose(newclone.H0, cosmo.H0)
    assert np.allclose(newclone.H0, 65.0)
    assert np.allclose(newclone.Om0, cosmo.Om0)
    assert np.allclose(newclone.Ok0, cosmo.Ok0)
    assert not np.allclose(newclone.Ogamma0, cosmo.Ogamma0)
    assert not np.allclose(newclone.Onu0, cosmo.Onu0)
    assert not np.allclose(newclone.Tcmb0, cosmo.Tcmb0)
    assert np.allclose(newclone.Tcmb0, 2.8)
    assert np.allclose(newclone.m_nu, cosmo.m_nu)
    assert np.allclose(newclone.Neff, cosmo.Neff)

    # And direct comparison
    cmp = core.FlatLambdaCDM(name="New name", H0=65, Om0=0.27, Tcmb0=2.8)
    assert newclone.__class__ == cmp.__class__
    assert newclone.name == cmp.name
    assert np.allclose(newclone.H0, cmp.H0)
    assert np.allclose(newclone.Om0, cmp.Om0)
    ...
    cosmo = core.w0waCDM(name="test w0wa", H0=70,
                         Om0=0.27, Ode0=0.5, wa=0.1, Tcmb0=4.0)
    newclone = cosmo.clone(w0=-1.1, wa=0.2)
    assert newclone.__class__ == cosmo.__class__
    assert newclone.name == cosmo.name
    assert np.allclose(newclone.H0, cosmo.H0)
    assert np.allclose(newclone.Om0, cosmo.Om0)
...
def test_xtfuncs():
    ...
    z = np.array([2.0, 3.2])
    assert np.allclose(cosmo.lookback_time_integrand(3), 0.052218976654969378,
                       rtol=1e-4)
    assert np.allclose(cosmo.lookback_time_integrand(z),
                       [0.10333179, 0.04644541], rtol=1e-4)
    assert np.allclose(cosmo.abs_distance_integrand(3), 3.3420145059180402,
                       rtol=1e-4)
    assert np.allclose(cosmo.abs_distance_integrand(z),
...
def test_repr():
    ...
    cosmo = core.LambdaCDM(70, 0.3, 0.5, Tcmb0=2.725)
    expected = ('LambdaCDM(H0=70, Om0=0.3, Ode0=0.5, Tcmb0=2.725, Neff=3.04, '
                'm_nu=[0. 0. 0.], Ob0=None)')
    assert str(cosmo) == expected

    cosmo = core.LambdaCDM(70, 0.3, 0.5, Tcmb0=2.725, m_nu=0.01)
    expected = ('LambdaCDM(H0=70, Om0=0.3, Ode0=0.5, Tcmb0=2.725, Neff=3.04, '
                'm_nu=[0.01 0.01 0.01], Ob0=None)')
    assert str(cosmo) == expected

    cosmo = core.FlatLambdaCDM(50.0, 0.27, Tcmb0=3, Ob0=0.05)
    expected = ('FlatLambdaCDM(H0=50, Om0=0.27, Tcmb0=3, Neff=3.04, '
                'm_nu=[0. 0. 0.], Ob0=0.05)')

    assert str(cosmo) == expected

    cosmo = core.wCDM(60.0, 0.27, 0.6, Tcmb0=2.725, w0=-0.8, name='test1')
    expected = ('wCDM(name="test1", H0=60, Om0=0.27, Ode0=0.6, w0=-0.8, '
                'Tcmb0=2.725, Neff=3.04, m_nu=[0. 0. 0.], Ob0=None)')
    assert str(cosmo) == expected

    cosmo = core.FlatwCDM(65.0, 0.27, w0=-0.6, name='test2')
    expected = ('FlatwCDM(name="test2", H0=65, Om0=0.27, w0=-0.6, Tcmb0=0, '
                'Neff=3.04, m_nu=None, Ob0=None)')
    assert str(cosmo) == expected

    cosmo = core.w0waCDM(60.0, 0.25, 0.4, w0=-0.6, Tcmb0=2.725, wa=0.1, name='test3')
    expected = ('w0waCDM(name="test3", H0=60, Om0=0.25, Ode0=0.4, w0=-0.6, '
                'wa=0.1, Tcmb0=2.725, Neff=3.04, m_nu=[0. 0. 0.], Ob0=None)')
    assert str(cosmo) == expected

    cosmo = core.Flatw0waCDM(55.0, 0.35, w0=-0.9, wa=-0.2, name='test4',
                             Ob0=0.0456789)
    expected = ('Flatw0waCDM(name="test4", H0=55, Om0=0.35, w0=-0.9, Tcmb0=0, '
                'Neff=3.04, m_nu=None, Ob0=0.0457)')
    assert str(cosmo) == expected

    cosmo = core.wpwaCDM(50.0, 0.3, 0.3, wp=-0.9, wa=-0.2,
                         zp=0.3, name='test5')
    expected = ('wpwaCDM(name="test5", H0=50, Om0=0.3, Ode0=0.3, wp=-0.9, '
                'wa=-0.2, zp=0.3, Tcmb0=0, Neff=3.04, m_nu=None, Ob0=None)')
    assert str(cosmo) == expected

    cosmo = core.w0wzCDM(55.0, 0.4, 0.8, w0=-1.05, wz=-0.2, Tcmb0=2.725,
                         m_nu=[0.001, 0.01, 0.015])
    expected = ('w0wzCDM(H0=55, Om0=0.4, Ode0=0.8, w0=-1.05, wz=-0.2 '
                'Tcmb0=2.725, Neff=3.04, m_nu=[0.001 0.01  0.015], Ob0=None)')
    assert str(cosmo) == expected
...
def test_flat_z1():
    ...
    assert np.allclose(cosmo.lookback_distance(z),
...
def test_zeroing():
    ...
    assert np.allclose(cosmo.Ode([1, 1, 2, 3]), [0, 0, 0, 0])
    assert np.allclose(cosmo.Ode(1), 0)
    # Ogamma0 and Onu
    cosmo = core.FlatLambdaCDM(H0=70, Om0=0.27, Tcmb0=0.0)
    assert np.allclose(cosmo.Ogamma(1.5), [0, 0, 0, 0])
    assert np.allclose(cosmo.Ogamma([0, 1, 2, 3]), [0, 0, 0, 0])
    assert np.allclose(cosmo.Onu(1.5), [0, 0, 0, 0])
    assert np.allclose(cosmo.Onu([0, 1, 2, 3]), [0, 0, 0, 0])
    # Obaryon
...
class test_cos_subnu(core.FLRW):
    def __init__(self):
        core.FLRW.__init__(self, 70.0, 0.27, 0.73, Tcmb0=3.0, m_nu=0.1,
                           name="test_cos_nu")
        self._w0 = -0.8
...
def test_matter():
    ...
    assert np.allclose(tcos.H0, 70.0)
    assert np.allclose(tcos.Om(0), 0.3)
    assert np.allclose(tcos.Ob(0), 0.045)
    z = np.array([0.0, 0.5, 1.0, 2.0])
    assert np.allclose(tcos.Om(z), [0.3, 0.59124088, 0.77419355, 0.92045455],
                       rtol=1e-4)
    assert np.allclose(tcos.Ob(z),
                       [0.045, 0.08868613, 0.11612903, 0.13806818], rtol=1e-4)
    assert np.allclose(tcos.Odm(z), [0.255, 0.50255474, 0.65806452, 0.78238636],
                       rtol=1e-4)
...
def test_ocurv():
    ...
    # Not flat
    tcos = core.LambdaCDM(70.0, 0.3, 0.5, Tcmb0=0.0)
...
def test_ogamma():
    ...
    # And integers for z
    assert np.allclose(cosmo.comoving_distance(z.astype(int)),
                       targvals, rtol=1e-5)
...
def test_tcmb():
    ...
    assert np.allclose(cosmo.Tcmb0, 2.5)
    assert np.allclose(cosmo.Tcmb(2), 7.5)
    z = [0.0, 1.0, 2.0, 3.0, 9.0]
    assert np.allclose(cosmo.Tcmb(z),
...
def test_tnu():
    ...
    assert np.allclose(cosmo.Tnu0, 2.1412975665108247, rtol=1e-6)
    assert np.allclose(cosmo.Tnu(2), 6.423892699532474, rtol=1e-6)
    z = [0.0, 1.0, 2.0, 3.0]
...
def test_efunc_vs_invefunc():
    ...
    cosmo = core.LambdaCDM(70, 0.3, 0.5, m_nu=0.01)
    assert np.allclose(cosmo.efunc(z0), 1.0 / cosmo.inv_efunc(z0))
    assert np.allclose(cosmo.efunc(z), 1.0 / cosmo.inv_efunc(z))
...
def test_kpc_methods():
    ...
    assert np.allclose(cosmo.arcsec_per_kpc_comoving(3), 0.0317179167)
...
def test_comoving_volume():
    ...
    assert np.allclose(c_flat.comoving_volume(redshifts), wright_flat,
                       rtol=1e-2)
    assert np.allclose(c_open.comoving_volume(redshifts),
                       wright_open, rtol=1e-2)
    assert np.allclose(c_closed.comoving_volume(redshifts),
...
def test_differential_comoving_volume():
    ...
    # The wright calculator isn't very accurate, so we use a rather
    # modest precision.
    ftemp = lambda x: c_flat.differential_comoving_volume(x)
    otemp = lambda x: c_open.differential_comoving_volume(x)
    ctemp = lambda x: c_closed.differential_comoving_volume(x)
...
def test_wz():
    ...
    assert np.allclose(cosmo.w(1.0), -1.25)
    assert np.allclose(cosmo.w([0.0, 0.5, 1.0, 1.5, 2.3]),
    ...
    assert np.allclose(cosmo.wa, 0.2)
    assert np.allclose(cosmo.zp, 0.5)
    assert np.allclose(cosmo.w(0.5), -0.9)
    assert np.allclose(cosmo.w([0.1, 0.2, 0.5, 1.5, 2.5, 11.5]),
...
def test_age():
    ...
    assert np.allclose(tcos.hubble_time, 13.889094057856937)
    assert np.allclose(tcos.age(4), 1.5823603508870991)
    ...
    assert np.allclose(tcos.age(4), 1.5773003779230699)
    assert np.allclose(tcos.age([1, 5]), [5.96344942, 1.20093077])

    # And massive neutrinos
    tcos = core.FlatLambdaCDM(70.4, 0.272, Tcmb0=3.0, m_nu=0.1)
    assert np.allclose(tcos.age(4), 1.5546485439853412)
    assert np.allclose(tcos.age([1, 5]), [5.88448152, 1.18383759])
...
def test_distmod():
    # WMAP7 but with Omega_relativisitic = 0
    tcos = core.FlatLambdaCDM(70.4, 0.272, Tcmb0=0.0)
    assert np.allclose(tcos.hubble_distance, 4258.415596590909)
...
def test_neg_distmod():
    ...
    assert np.allclose(tcos.luminosity_distance([50, 100]),
                       [16612.44047622, -46890.79092244])
    assert np.allclose(tcos.distmod([50, 100]),
...
def test_critical_density():
    ...
    tcos = core.FlatLambdaCDM(70.4, 0.272, Tcmb0=0.0)
    assert np.allclose(tcos.critical_density0, 9.309668456020899e-30)
    assert np.allclose(tcos.critical_density0, tcos.critical_density(0))
...
def test_comoving_distance_z1z2():
    ...
    assert np.allclose(tcos._comoving_distance_z1z2(1, 2),
    ...
    assert np.allclose(tcos._comoving_distance_z1z2(z1, z2), results)
...
def test_age_in_special_cosmologies():
    ...
    assert np.allclose(c_dS.age(z=0), np.inf)
    assert np.allclose(c_dS.age(z=1), np.inf)
    assert np.allclose(c_dS.lookback_time(z=0), 0)
    assert np.allclose(c_dS.lookback_time(z=1), 6.777539216261741)

    c_EdS = core.FlatLambdaCDM(100, 1, Tcmb0=0)
    assert np.allclose(c_EdS.age(z=0), 6.518614811154189)
    assert np.allclose(c_EdS.age(z=1), 2.3046783684542738)
    assert np.allclose(c_EdS.lookback_time(z=0), 0)
    assert np.allclose(c_EdS.lookback_time(z=1), 4.213936442699092)
...
def test_distance_in_special_cosmologies():
    ...
    """
    c_dS = core.FlatLambdaCDM(100, 0, Tcmb0=0)
    assert np.allclose(c_dS.comoving_distance(z=0), 0)
    assert np.allclose(c_dS.comoving_distance(z=1), 2997.92458)

    c_EdS = core.FlatLambdaCDM(100, 1, Tcmb0=0)
    assert np.allclose(c_EdS.comoving_distance(z=0), 0)
    assert np.allclose(c_EdS.comoving_distance(z=1), 1756.1435599923348)
...
def test_comoving_transverse_distance_z1z2():
    ...
    assert np.allclose(tcos._comoving_transverse_distance_z1z2(1, 2),
                       1313.2232194828466)
    ...
    assert np.allclose(tcos._comoving_distance_z1z2(z1, z2),
    ...
    assert np.allclose(tcos._comoving_transverse_distance_z1z2(z1, z2),
                       results)
...
def test_angular_diameter_distance_z1z2():
    ...
    results = (0.,
               ...
               1508.37010062,
               1621.07937976)
    assert np.allclose(tcos.angular_diameter_distance_z1z2(0.1, z2),
                       results)
...
def test_massivenu_basic():
    ...
    tcos = core.FlatLambdaCDM(70.4, 0.272, Neff=4.05, Tcmb0=2.725, m_nu=0.)
    assert np.allclose(tcos.Neff, 4.05)
    assert not tcos.has_massive_nu
    mnu = tcos.m_nu
    assert len(mnu) == 4
    assert np.allclose(mnu, [0.0, 0.0, 0.0, 0.0])
    ...
    tcos = core.FlatLambdaCDM(70.4, 0.272, Tcmb0=0, m_nu=0.4)
    assert not tcos.has_massive_nu
    assert tcos.m_nu is None

    # Test basic setting, retrieval of values
    tcos = core.FlatLambdaCDM(70.4, 0.272, Tcmb0=2.725, m_nu=[0.0, 0.01, 0.02])
    assert tcos.has_massive_nu
    mnu = tcos.m_nu
    assert len(mnu) == 3
    assert np.allclose(mnu, [0.0, 0.01, 0.02])

    # All massive neutrinos case
    tcos = core.FlatLambdaCDM(70.4, 0.272, Tcmb0=2.725, m_nu=0.1, Neff=3.1)
    assert np.allclose(tcos.Neff, 3.1)
    assert tcos.has_massive_nu
    mnu = tcos.m_nu
    assert len(mnu) == 3
    assert np.allclose(mnu, [0.1, 0.1, 0.1])
...
def test_z_at_value():
    ...
    assert np.allclose(z_at_value(cosmo.luminosity_distance, d), 3, rtol=1e-8)
    assert np.allclose(z_at_value(cosmo.age, 2), 3.198122684356, rtol=1e-6)
    assert np.allclose(z_at_value(cosmo.luminosity_distance, 1e4),
                       1.3685790653802761, rtol=1e-6)
    assert np.allclose(z_at_value(cosmo.lookback_time, 7),
                       0.7951983674601507, rtol=1e-6)
    assert np.allclose(z_at_value(cosmo.angular_diameter_distance, 1500,
                                  zmax=2), 0.68127769625288614, rtol=1e-6)
    assert np.allclose(z_at_value(cosmo.angular_diameter_distance, 1500,
                                  zmin=2.5), 3.7914908028272083, rtol=1e-6)
    assert np.allclose(z_at_value(cosmo.distmod, 46),
    ...
    with pytest.raises(core.CosmologyError):
        with pytest.warns(UserWarning, match=r'fval is not bracketed'):
            z_at_value(cosmo.angular_diameter_distance, 1500, zmax=0.5)

    with pytest.raises(core.CosmologyError):
        with pytest.warns(UserWarning, match=r'fval is not bracketed'):
            z_at_value(cosmo.angular_diameter_distance, 1500, zmin=4.)
...
def test_elliptic_comoving_distance_z1z2():
    """Regression test for astropy/astropy#8388."""
    cosmo = core.LambdaCDM(70., 2.3, 0.05, Tcmb0=0)
    z = 0.2
    assert np.allclose(cosmo.comoving_distance(z),
                       cosmo._integral_comoving_distance_z1z2(0., z))
    assert np.allclose(cosmo._elliptic_comoving_distance_z1z2(0., z),
                       cosmo._integral_comoving_distance_z1z2(0., z))
